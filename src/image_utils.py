...
def srgb2lin(srgb:np.ndarray)->np.ndarray:
    ...
    return np.where(
        srgb <= 0.0404482362771082, srgb / 12.92, np.power(((srgb + 0.055) / 1.055), 2.4)
    )
...
def take_luminance_from_first_chroma_from_second(luminance:np.ndarray, chroma:np.ndarray, mode:str="lab", scaling_factor:int=1)->np.ndarray:
    ...
    if mode == "luminance":
        ...
        return lin2srgb(
            np.clip(
                lchroma
                * ((get_luminance(lluminance) / (get_luminance(lchroma))) ** scaling_factor)[
                    :, :, np.newaxis
                ],
                0,
                1,
