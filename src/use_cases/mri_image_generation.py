...
class MriImageGeneration:
    ...
    def __init__(
        ...
        self._model.load_state_dict(
            load_state_dict(checkpoint_path, location=self._device)
        )
    ...
    def process_sample(
        ...
        with torch.no_grad():
            ...
            cond = {
                "c_concat": [control],
                "c_crossattn": [
                    self._model.get_learned_conditioning(
            ...
            un_cond = {
                ...
                "c_crossattn": [
                    self._model.get_learned_conditioning([n_prompt] * num_samples)
                ],
            }
            ...
            self._model.control_scales = (
                [strength * (0.825 ** float(12 - i)) for i in range(13)]
                if guess_mode
                else ([strength] * 13)
            )  # Magic number. IDK why. Perhaps because 0.825**12<0.01 but 0.826**12>0.01
            samples, _ = self._ddim_sampler.sample(
                ddim_steps,
                num_samples,
                shape,
                cond,
    ...
    def gradio_ui(self) -> gr.Blocks:
        ...
        with block:
            with gr.Row():
                gr.Markdown(
                    "## MRI Image Generation with Control Net and Canny Edge Maps"
                )
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Accordion("Advanced options", open=False):
                        ...
                        ddim_steps = gr.Slider(
                            label="Steps", minimum=1, maximum=100, value=10, step=1
                        )
                        scale = gr.Slider(
                        ...
                        a_prompt = gr.Textbox(
                            label="Added Prompt", value="good quality"
                        )
                        n_prompt = gr.Textbox(
            ...
            run_button.click(
                fn=partial(MriImageGeneration.process_sample, self),
                inputs=ips,
